...
import MasterSupplier from '../../build/MasterSupplier.json'
...
export const TOKEN_NAME = "Power"
export const TOKEN_SYMBOL = "POWER"
...
export async function deployMasterSupplier(
  wallets: Wallet[],
  token: Contract,
  ...
  const contract = await deployContract(alice, MasterSupplier, [
    token.address,
    dev.address,
  ...
  //Init contract
  if (initializeLocks) {
    await contract.lockUpdate(95) // 95% of rewards will be locked between the Power token's _lockFromBlock and _lockToBlock
