...
import { deployMasterSupplier, deployGovernanceToken, TOKEN_NAME, TOKEN_SYMBOL, TOTAL_CAP, MANUAL_MINT_LIMIT } from '../shared/deploy'
...
// MasterSupplier halving settings
...
describe('MasterSupplier::Rewards', () => {
  ...
  context("Entering & withdrawing from pools + claiming rewards", function () {
    ...
    it("should not pay out POWER rewards before farming has started", async function () {
      ...
      await expect(supplier.connect(bob).deposit(0, 0, ZERO_ADDRESS)).to.be.reverted

      await supplier.connect(bob).deposit(0, expandTo18Decimals(100), ZERO_ADDRESS)

      expect(await govToken.totalSupply()).to.equal(0)
      
      await supplier.connect(bob).claimReward(0)
      expect(await govToken.totalSupply()).to.equal(0)
      expect(await govToken.balanceOf(bob.address)).to.equal(expandTo18Decimals(0))
    })

    it("should pay out POWER rewards after farming has started", async function () {
      this.timeout(0)
      ...
      const activeMultiplier = await supplier.getMultiplier(currentBlock.number, currentBlock.number+1)
      const firstMultiplier = await supplier.REWARD_MULTIPLIER(0)
      expect(activeMultiplier).to.equal(firstMultiplier)

      const rewardPerBlock = await supplier.REWARD_PER_BLOCK()
      ...
      await expect(supplier.connect(bob).claimReward(0))
        .to.emit(supplier, 'SendGovernanceTokenReward') // emit SendGovernanceTokenReward(msg.sender, _pid, pending, lockAmount);
      ...
      const { forDev, forFarmer, forLP, forCom, forFounders } = await supplier.getPoolReward(currentBlock.number, currentBlock.number+1, rewardsPerBlock)
      ...
      await supplier.add(rewardsPerBlock*rewardsMultiplierForSecondPool, lp2.address, true) //5x bonus rewards pool vs pool 0
      await lp2.connect(carol).approve(supplier.address, expandTo18Decimals(1000))
      await supplier.connect(carol).deposit(1, expandTo18Decimals(100), ZERO_ADDRESS)

      // she should have two times (two sets of rewards since we're at block 102) 5x (=10x) of Bob's block 101 rewards
      await expect(supplier.connect(carol).claimReward(1))
        .to.emit(supplier, 'SendGovernanceTokenReward') // emit SendGovernanceTokenReward(msg.sender, _pid, pending, lockAmount);
      ...
      await lp2.connect(bob).approve(supplier.address, expandTo18Decimals(1000))
      await supplier.connect(bob).deposit(1, expandTo18Decimals(100), ZERO_ADDRESS)

      // Advance 10 blocks, then claim rewards from all pools
      advanceBlockWith(provider, 10)
      await supplier.connect(bob).claimRewards([0, 1])

      expect('claimReward').to.be.calledOnContractWith(supplier, [0]);
      expect('claimReward').to.be.calledOnContractWith(supplier, [1]);
    ...
    it("should not distribute POWER if no one deposit", async function () {
      this.timeout(0)
      const debugMessages = false
      // 1 per block farming rate starting at block 600 with the first halvening block starting 1000 blocks after the start block
      const rewardsPerBlock = 1
      const supplier = await deployMasterSupplier(wallets, govToken, expandTo18Decimals(rewardsPerBlock), 600, 1000)
      await govToken.transferOwnership(supplier.address)
      await supplier.add(rewardsPerBlock, lp.address, true)
      await lp.connect(bob).approve(supplier.address, expandTo18Decimals(1000))
      ...
      // block 610: 
      await expect(supplier.connect(bob).deposit(0, expandTo18Decimals(10), ZERO_ADDRESS)) 
        .to.emit(supplier, 'Deposit') //emit Deposit(msg.sender, _pid, _amount);
      ...
      await expect(supplier.connect(bob).withdraw(0, expandTo18Decimals(10), ZERO_ADDRESS)).to.be.reverted

      // calculate the user's deposit
      const userDepositFee = await supplier.userDepFee()
      ...
      await expect(supplier.connect(bob).withdraw(0, likelyDeposit, ZERO_ADDRESS)) 
        .to.emit(supplier, 'Withdraw') //emit Withdraw(msg.sender, _pid, _amount);
    ...
    it("should distribute POWER properly for each staker"), async () => {
      // 1 per block farming rate starting at block 300 with the first halvening block starting 1000 blocks after the start block
      const rewardsPerBlock = 1
      const supplier = await deployMasterSupplier(wallets, govToken, expandTo18Decimals(rewardsPerBlock), 300, 1000)

      await govToken.transferOwnership(supplier.address)
      await supplier.add(rewardsPerBlock, lp.address, true)
      await lp.connect(alice).approve(supplier.address, expandTo18Decimals(1000))
      await lp.connect(bob).approve(supplier.address, expandTo18Decimals(1000))
      await lp.connect(carol).approve(supplier.address, expandTo18Decimals(1000))
      // Alice deposits 10 LPs at block 310
      await advanceBlockTo(provider, 309)
      await supplier.connect(alice).deposit(0, expandTo18Decimals(10), ZERO_ADDRESS)
      // Bob deposits 20 LPs at block 314
      await advanceBlockTo(provider, 313)
      await supplier.connect(bob).deposit(0, expandTo18Decimals(20), ZERO_ADDRESS)
      // Carol deposits 30 LPs at block 318
      await advanceBlockTo(provider, 317)
      await supplier.connect(carol).deposit(0, expandTo18Decimals(30), ZERO_ADDRESS)
      ...
      expect(await govToken.balanceOf(supplier.address)).to.equal(expandTo18Decimals(4334))
      ...
      await supplier.connect(bob).withdraw(0, expandTo18Decimals(5), ZERO_ADDRESS)
      ...
      expect(await govToken.balanceOf(carol.address)).to.equal(0)
      expect(await govToken.balanceOf(supplier.address)).to.equal(expandTo18Decimals(8144))
      ...
      await supplier.connect(alice).withdraw(0, expandTo18Decimals(20), ZERO_ADDRESS)
      await advanceBlockTo(provider, 349)
      await supplier.connect(bob).withdraw(0, expandTo18Decimals(15), ZERO_ADDRESS)
      await advanceBlockTo(provider, 359)
      await supplier.connect(carol).withdraw(0, expandTo18Decimals(30), ZERO_ADDRESS)
    ...
    it("should give proper POWER allocation to each pool"), async () => {
      ...
      const rewardsPerBlock = 1
      const supplier = await deployMasterSupplier(wallets, govToken, expandTo18Decimals(rewardsPerBlock), 400, 1000)
      ...
      await supplier.add(rewardsPerBlock*2, lp2.address, true) // 2x bonus
      // Alice should have 10*1000 pending reward
      expect(await supplier.pendingReward(0, alice.address)).to.equal(expandTo18Decimals(10000))
      // Bob deposits 10 LP2s at block 425
      await advanceBlockTo(provider, 424)
      await supplier.connect(bob).deposit(1, expandTo18Decimals(5), ZERO_ADDRESS)
      // Alice should have 10000 + 5*1/3*1000 = 11666 pending reward
      expect(await supplier.pendingReward(0, alice.address)).to.equal(expandTo18Decimals(11666))
      await advanceBlockTo(provider, 430)
      // At block 430. Bob should get 5*2/3*1000 = 3333. Alice should get ~1666 more.
      expect(await supplier.pendingReward(0, alice.address)).to.equal(expandTo18Decimals(13333))
      expect(await supplier.pendingReward(1, bob.address)).to.equal(expandTo18Decimals(3333))
    }
  })
