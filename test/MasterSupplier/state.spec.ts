...
import { deployMasterSupplier } from '../shared/deploy'

import GovernanceToken from '../../build/GovernanceToken.json'

chai.use(solidity)

describe('MasterSupplier::State', () => {
  ...
  context("Ethereum", function () {
    ...
    // Multipliers have also been significantly modified to suit POWER:s emission model
    ...
    beforeEach(async () => {
      govToken = await deployContract(alice, GovernanceToken, ["Power", "POWER", TOTAL_CAP, MANUAL_MINT_LIMIT, lockFromBlock, lockToBlock])
      supplier = await deployMasterSupplier(wallets, govToken, expandTo18Decimals(1000), rewardsStartBlock, halvingAfterBlockCount)
      await govToken.transferOwnership(supplier.address)
    })

    it("should set correct state variables", async function () {
      const usedGovToken = await supplier.govToken()
      const devaddr = await supplier.devaddr()
      const liquidityaddr = await supplier.liquidityaddr()
      const comfundaddr = await supplier.comfundaddr()
      const founderaddr = await supplier.founderaddr()
      ...
      expect(owner).to.equal(supplier.address)
    })
    ...
    it("should calculate correct values for multipliers, rewards halvings and finish bonus block", async function () {
      ...
      for (let i = 0; i < rewardMultipliers.length - 1; i++) {
        expect(await supplier.REWARD_MULTIPLIER(i)).to.equal(rewardMultipliers[i])
        const halvingBlock = await supplier.HALVING_AT_BLOCK(i)
      ...
      expect(await supplier.FINISH_BONUS_AT_BLOCK()).to.equal(expectedFinishBonusAtBlock)
    })
    ...
    it("should correctly calculate reward multipliers for all halving blocks", async function () {
      ...
      for (let i = 0; i < halvingAtBlocks.length; i++) {
        ...
        expect(await supplier.HALVING_AT_BLOCK(i)).to.equal(halvingAtBlock)

        const blockBefore = halvingAtBlock.sub(1)
        const multiplier = await supplier.getMultiplier(blockBefore, halvingAtBlock)
        expect(await supplier.REWARD_MULTIPLIER(i)).to.equal(multiplier)
    ...
    it("should correctly update HALVING_AT_BLOCK using halvingUpdate", async function () {
      ...
      await supplier.halvingUpdate(updatedHalvingAtBlocks)
  ...
  context("Harmony", function () {
    ...
    it("should calculate correct values for multipliers, rewards halvings and finish bonus block", async function () {
      ...
      for (let i = 0; i < rewardMultipliers.length - 1; i++) {
        const rewardMultiplier = await supplier.REWARD_MULTIPLIER(i)
        const halvingBlock = await supplier.HALVING_AT_BLOCK(i)
        halvingAtBlocks.push(halvingBlock)
